# ============================================================
# 🔷 Import DLT / SDP Module
# ============================================================

# Used to define views, tables, and CDC flows in Spark Declarative Pipelines
import dlt as dp


# ============================================================
# 1. Dimension View: Passenger
# ============================================================

@dp.view
def dim_passenger_view():

    # Step 1: Read streaming data from Silver OBT
    # This table already contains cleaned + enriched data
    df = spark.readStream.table("silver_obt")

    # Step 2: Select only passenger-related columns
    # This defines the structure of passenger dimension
    df = df.select(
        "passenger_id",
        "passenger_name",
        "passenger_email",
    ...
    # Step 3: Remove duplicate passengers
    # Ensures one unique record per passenger_id
...
# ============================================================
# 2. Create Target Dimension Table
# ============================================================

# Creates an empty streaming dimension table
# Data will be inserted using CDC flow
dp.create_streaming_table("dim_passenger")


# ============================================================
# 3. Apply CDC (Slowly Changing Dimension Logic)
# ============================================================
...
dp.create_auto_cdc_flow(
    ...
    # Source view (defined above)
    source="dim_passenger_view",

    # Primary key for dimension
    keys=["passenger_id"],

    # Column used for ordering updates
    # (used to identify latest record)
    sequence_by="passenger_id",

    # SCD Type 1 → overwrite old data with latest
    stored_as_scd_type=1,
)

## do same for all dim table as well 
...
def dim_booking_view():
    df = spark.readStream.table("silver_obt")
    df = df.select("ride_id","confirmation_number","ride_status_id","pickup_location_id","pickup_address","pickup_latitude","pickup_longitude","dropoff_location_id","dropoff_city_id","dropoff_address","dropoff_latitude","dropoff_longitude","booking_timestamp","dropoff_timestamp","cancellation_reason_id")
